...
class Main
{
	private static final double alpha = 0.04286;
	public static final double beta = 1.5;
	private static final double gamma = 57.12;
	// 初期値設定
	// setflw
	// レイノルズ数
	...
	static final int j1 = 96;
	static final int j2 = 106;
	static final int j3 = 96;
	static final int j4 = 106;
	...
	public static void setgrd() {
		...
		for (int i = i2+1; i <= mx; i++) {
			for (int j = 1; j <= my; j++) {
				x[i][j] = alpha * Math.pow(i,beta) + gamma;
				y[i][j] = dy * (double)( j - jcent );
			}
		}
...
// (3) 最終結果の書き出し
	...
	for (int i = 2; i <= mx-1; i++) {
		...
		if (outbox(i,j)) { //物体の内部は計算しない
		    ux = ( u[i+1][j] - u[i-1][j] ) / ( 2.0 * (x[i+1][j] - x[i][j]));
		    uy = ( u[i][j+1] - u[i][j-1] ) / ( 2.0 * dy );
		    vx = ( v[i+1][j] - v[i-1][j] ) / ( 2.0 * (x[i+1][j] - x[i][j]) );
	...
	for (int itr = 1; itr <= maxitp; itr ++) {
		...
		for (int j = 2; j <= my-1; j++) {
		    	if (outbox(i,j)) { //物体の内部は計算しない
			    dp = ( p[i+1][j] + p[i-1][j] ) / ( (x[i+1][j] - x[i][j]) * (x[i+1][j] - x[i][j]) )
				+ ( p[i][j+1] + p[i][j-1] ) / ( dy * dy )
				- rhs[i][j];
			    dp = dp / ( 2.0 / ((x[i+1][j] - x[i][j]) * (x[i+1][j] - x[i][j])) + 2.0 / (dy * dy) ) - p[i][j];
			    res = res + dp * dp; //残渣
	...
	// (1) 圧力勾配
	    ...
	    for (int j = 1; j < my+1; j++) {
	        if (outbox(i,j)) { //物体の内部は計算しない
		    urhs[i][j] = -( p[i+1][j] - p[i-1][j] ) / ( 2.0 * (x[i+1][j] - x[i][j]) );
	...
	// (2) 粘性項
	    ...
	    for (int j = 1; j < my+1; j++) {
	        if (outbox(i,j)) { //物体の内部は計算しない
		    urhs[i][j] = urhs[i][j]
			+ ( u[i+1][j] - 2.0 * u[i][j] + u[i-1][j] ) / ( re * (x[i+1][j] - x[i][j]) * (x[i+1][j] - x[i][j]) )
			+ ( u[i][j+1] - 2.0 * u[i][j] + u[i][j-1] ) / ( re * dy * dy );
		    vrhs[i][j] = vrhs[i][j]
			+ ( v[i+1][j] - 2.0 * v[i][j] + v[i-1][j] ) / ( re * (x[i+1][j] - x[i][j]) * (x[i+1][j] - x[i][j]) )
	...
	for (int i = 2; i < mx; i++) {
	    for (int j = 2; j < my; j++) {
	        if (outbox(i,j)) { //物体の内部は計算しない
		    urhs[i][j] = urhs[i][j]
			-u[i][j] * (-u[i+2][j] + 8.0 * ( u[i+1][j] - u[i-1][j] ) + u[i-2][j]) / (12.0 * (x[i+1][j] - x[i][j]))
			-Math.abs(u[i][j]) * ( u[i+2][j] - 4.0 * u[i+1][j] + 6.0 * u[i][j] - 4.0 * u[i-1][j] + u[i-2][j] ) / ( 4.0 * (x[i+1][j] - x[i][j]));
		    vrhs[i][j] = vrhs[i][j]
			-u[i][j] * (-v[i+2][j] + 8.0 * ( v[i+1][j] - v[i-1][j] ) + v[i-2][j]) / (12.0 * (x[i+1][j] - x[i][j]))
			-Math.abs(u[i][j]) * ( v[i+2][j] - 4.0 * v[i+1][j] + 6.0 * v[i][j] - 4.0 * v[i-1][j] + v[i-2][j] ) / ( 4.0 * (x[i+1][j] - x[i][j]));
		}
	    }
	}
