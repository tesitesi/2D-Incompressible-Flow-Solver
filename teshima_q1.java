...
class Main
{
	...
	FileWriter fwu = new FileWriter("./KarmanU.csv", false);
	PrintWriter pwu = new PrintWriter(new BufferedWriter(fwp));
	FileWriter fwv = new FileWriter("./KarmanV.csv", false);
	PrintWriter pwv = new PrintWriter(new BufferedWriter(fwp));
	FileWriter fwo = new FileWriter("./KarmanOmega.csv", false);
	PrintWriter pwo = new PrintWriter(new BufferedWriter(fwp));
	...
	for (int i = 2; i <= mx-1; i++) {
		...
		if ( ( i <= i1 || i2 <= i ) || ( j <= j1 || j2 <= j ) ) { //物体の内部は計算しない
